...
def update_service(update_service, service_id, current_user_id):
    service = Services.query.get(service_id)
    service_id = service.company.user_id
    
    if current_user_id == service_id:
        updated_service = Repository.update_service(update_service, service_id, service)
